...
namespace planners
{
        ...
        formalism::State &final_state)
    {
        ...
        ++expanded;
        const auto grounding_time_start = std::chrono::high_resolution_clock::now();
        const auto applicable_actions = successor_generator_->get_applicable_actions(initial_state);
        const auto grounding_time_end = std::chrono::high_resolution_clock::now();
    ...
    void IWSearch::print_statistics() const {
        std::cout << "Expanded " << expanded << " states" << std::endl;
        std::cout << "Generated " << generated << " states" << std::endl;
        std::cout << "Pruned " << pruned << " states" << std::endl;
        std::cout << "Effective width " << effective_arity << std::endl;
        std::cout << "Successor time: " << time_successors_ns / (int64_t) 1E6 << " ms"
                << " (" << std::fixed << std::setprecision(3) << (100.0 * time_successors_ns) / time_total_ns << "%)" << std::endl;
        std::cout << "Apply time: " << time_apply_ns / (int64_t) 1E6 << " ms"
                << " (" << std::fixed << std::setprecision(3) << (100.0 * time_apply_ns) / time_total_ns << "%)" << std::endl;
        std::cout << "Grounding time: " << time_grounding_ns / (int64_t) 1E6 << " ms"
                    << " (" << std::fixed << std::setprecision(3) << (100.0 * time_grounding_ns) / time_total_ns << "%)" << std::endl;
        std::cout << "Goal time: " << time_goal_test_ns / (int64_t) 1E6 << " ms"
                << " (" << std::fixed << std::setprecision(3) << (100.0 * time_goal_test_ns) / time_total_ns << "%)" << std::endl;
        goal_test_->print_statistics();
        std::cout << "Search time: " << time_search_ns / (int64_t) 1E6 << " ms" << std::endl;
        std::cout << "Total time: " << time_total_ns / (int64_t) 1E6 << " ms" << std::endl;
    }
