...
#include "../private/planners/serialized_search.hpp"
#include "../private/planners/goal_test.hpp"
#include "../private/planners/dlplan_utils.hpp"
...
int main(int argc, char* argv[]) {
    if (argc != 6) {
        throw std::runtime_error("Correct usage: ./siwr <domain:str> <problem:str> <arity:int> <sketch:str> <enable_grounding:bool>");
    }
    std::cout << "Started siwr search." << std::endl;

    std::string domain_filename(argv[1]);
    std::string problem_filename(argv[2]);
    int arity = atoi(argv[3]);
    std::string sketch_filename(argv[4]);
    bool enable_grounding = utils::to_bool(argv[5]);
    ...
    auto vocabulary = planners::construct_vocabulary(domain);
    auto instance = planners::construct_instance(problem, vocabulary);
    auto sketch = planners::read_sketch(vocabulary, sketch_filename);
    auto goal_test = std::make_unique<planners::SketchGoalTest>(problem, instance, sketch);
    ...
    planners::SerializedSearch planner(
        problem,
        planners::IWSearch(problem, successor_generator_type, std::move(goal_test), arity));
