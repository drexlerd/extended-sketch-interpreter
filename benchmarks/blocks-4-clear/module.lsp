(:module
    ...
    (:extended_sketch
        ...
        (:numericals )
        (:concepts
            (C "c_primitive(clear_g,0)")
        )
        (:call_rule (:conditions (:memory m0)) (:effects (:memory m0) (:call clear((:concepts (C))))))
    )
)
(:module
    (:signature clear((:concepts (C))))
        (:extended_sketch
        ...
        (:booleans
            ...
            (A "b_empty(c_and(c_some(r_transitive_closure(r_primitive(on,0,1)),c_argument(0)),c_register(1)))")
        )
        (:numericals )
        (:concepts
            (N "c_diff(c_argument(0),c_primitive(clear,0))")
