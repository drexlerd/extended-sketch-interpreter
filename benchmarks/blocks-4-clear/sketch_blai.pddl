(:extended_sketch
    ...
    (:numericals
        ...
        (nC "n_count(c_diff(c_primitive(clear_g,0),c_or(c_primitive(clear,0),c_primitive(holding,0))))")
    )
    (:concepts
        (C "c_diff(c_primitive(clear_g,0),c_or(c_primitive(clear,0),c_primitive(holding,0)))")
    )
